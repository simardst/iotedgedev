    ...
    def convert_create_options(self):
        ...
        for module_name, module_info in modules.items():
            if "settings" in module_info and "createOptions" in module_info["settings"]:
                ...
                if isinstance(create_options, six.string_types):
                    create_options_str = create_options
                else:
                    create_options_str = json.dumps(create_options)

                options = [m for m in re.finditer("(.|[\r\n]){{1,{0}}}".format(TWIN_VALUE_MAX_SIZE), create_options_str)]
